...
public class Logger extends BaseSystem implements ISystem
{
    ...
    private Logger()
    {
    	super(Module.Logger, SystemState.Active, Priority.Essential);    	
    	_instance = this;    
    }
	...
	public String ClearColor(String coloredMessage) 
	{	
		System.out.print("Colored : " +  coloredMessage);
		
		String message = "";
		coloredMessage = RemoveColor(coloredMessage);
		
		for(int i = 0 ; i < coloredMessage.length() ; i++)
		{
			char character = coloredMessage.charAt(i);
			
			System.out.print("Char at <" + i + "> is  <" +  character + ">");
		...
		System.out.print("Cleared : " +  message);
    ...
    {    
    	...
    	if(messageType != MessageType.None && addSystemTag)
    	{
    		...
    		case CoreManager:
    			outPutMessage += _config.Message.IO.CoreManager;
    			break;
    			
    		case DataBase:
    			outPutMessage += _config.Message.DataBase.DataBaseTag;
    			break;
    		...
    		case WorldSystem:
    			outPutMessage += _config.Message.World.WorldSystemTag;
    			break;
		...
		case Command:
			outPutMessage +=  _config.Message.IO.Command;
			break;
			
		case Error:
			outPutMessage +=  _config.Message.IO.Error;
			sound = Sound.BLOCK_NOTE_BLOCK_BASEDRUM;
			break;
			
		case Info:
			outPutMessage +=  _config.Message.IO.Info;
			sound = Sound.BLOCK_NOTE_BLOCK_BELL;
			break;
			
		case Loading:
			outPutMessage +=  _config.Message.IO.Loading;
			break;
		...
		case Offline:
			outPutMessage +=  _config.Message.IO.Offline;
			break;
			
		case Online:
			outPutMessage +=  _config.Message.IO.Online;
			break;
			
		case Question:
			outPutMessage +=  _config.Message.IO.Question;
			break;
			
		case Warning:
			outPutMessage +=  _config.Message.IO.Warning;
