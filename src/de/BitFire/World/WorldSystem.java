...
import de.BitFire.API.Bukkit.BukkitAPIWorld;
...
public class WorldSystem extends BaseSystem implements ISystem
{
	...
	private WorldSystem() 
	{
		super(Module.WorldSystem, SystemState.Active, Priority.Essential);
		_instance = this;
	...
	{		
		...
		else
		{
			...
			{
				String message = _config.Message.World.ErrorWhileCreatingWorld + _config.Message.IO.ErrorArrow + e.getMessage();
	...
	{
		List<World> loadedWorlds = BukkitAPIWorld.GetLoadedWorlds();
	...
	public void PrintData(CommandSender sender) 
	{
		...
		String message = _config.Message.IO.NewLine + _config.Message.World.WorldListHeader + _config.Message.IO.NewLine;
		String worldMessage = "";
		...
		if (noWorldsLoaded)
		{
			message += _config.Message.World.NoLoadedWorlds + _config.Message.IO.NewLine;
		}
		else
		{
			...
			for (CMWorld cmWorld : worlds)
			{
				...
				if(cmWorld.Information.MapEnvironment == null)
				{
					...
					case FailedToLoad:
						worldMessage = _config.Message.World.FailedToLoadWorld;
						break;
						
		
						
					case Undefined:
						worldMessage = _config.Message.World.UndefinedWorld;
						break;
						
					case ActiveAndRegistered:
						worldMessage = _config.Message.World.LoadedWorld;
						break;
					...
					worldMessage = _messageTags.ReplaceIDTag(worldMessage, cmWorld.Information.ID);
					...
					message += worldMessage + _config.Message.IO.NewLine;
		...
		message += _config.Message.World.WorldListFooter;

		message = message.replace(Environment.NORMAL.toString(), _config.Message.World.WorldNormal);
		message = message.replace(Environment.NETHER.toString(), _config.Message.World.WorldNether);
		message = message.replace(Environment.THE_END.toString(), _config.Message.World.WorldEnd);
	...
	public void UnloadMap(CMWorld cmWorld)
	{
		...
		isWorldUnloaded = BukkitAPIWorld.UnloadWorld(cmWorld.BukkitWorld);
		
		if(isWorldUnloaded)
		{
			BukkitAPIWorld.UnloadWorld(cmWorld.BukkitWorld);
	...
	public void SetSpawnLocation(CommandSender sender) throws Exception
	{
		...
		if (isSenderPlayer)
		{
			...
			try
			{
				...
				_logger.SendToSender(Module.WarpSystem, MessageType.Info, sender, _config.Message.TeleportSystem.SpawnUpdated);
			}
			catch (Exception e)
			{
				throw new Exception(_config.Message.TeleportSystem + "World.SetSpawnLocation Error\n" + e.getMessage());
			}
		}
	}
