...
public class Timber extends BaseSystem implements ISystem
{
	...
	private Timber()
	{
		super(Module.Timber, SystemState.Active, SystemPriority.Low);
		_instance = this;
	...
	{
		return _bukkitUtility.Block.IsWoodLog(block);
	}
	
	private boolean CheckIfLeeves(Block block)
	{
		return _bukkitUtility.Block.IsLeaveBlock(block);
	}
	...
	private void getBlocksToChop(Block block, Block highest, List<Block> blocks)
	{
		...
		while (block.getY() <= highest.getY())
		{
			...
			blockData =  _bukkitUtility.Block.GetID(block);			
	...
	private boolean breaksTool(Player player, ItemStack item)
	{
		...
		if(player.getGameMode() != GameMode.CREATIVE)		
		{
			if (item != null)
			{
				isItemAxe = _bukkitUtility.Item.IsItemAxe(item);			
	...
	*/
	...
	{		
		...
		{								
			hasTool = _bukkitUtility.Item.IsItemAxeEquiped(player);						
