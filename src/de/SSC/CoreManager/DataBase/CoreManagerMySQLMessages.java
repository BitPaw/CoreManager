...
//	private final String _ip = "{IP}";
	...
	{			
		return "select * from player where Cracked = " + (_bukkitUtility.Server.IsServerCracked() ? "1" : "0");
	}
	...
	public String DoesPlayerExist(Player player)
	{
		...
		sqlMessage = sqlMessage.replace(_isCracked, (_bukkitUtility.Server.IsServerCracked() ? "1" : "0"));
	...
	public String UpdatePlayer(CMPlayer cmPlayer)
	{
		...
		sqlMessage = sqlMessage.replace(_ipValue, _bukkitUtility.Player.GetPlayerIP(cmPlayer.BukkitPlayer));
	...
	public String UpdateRank(CMPlayer cmPlayer) 
	{
		...
		sqlMessage  = sqlMessage.replace(_newValue, cmPlayer.RankGroup.Name);
		sqlMessage  = sqlMessage.replace(_key, CMPlayerTable.UUID.toString());
		sqlMessage  = sqlMessage.replace(_keyValue, uuid.toString());
		sqlMessage = sqlMessage.replace(_isCracked, (_bukkitUtility.Server.IsServerCracked() ? "1" : "0"));
