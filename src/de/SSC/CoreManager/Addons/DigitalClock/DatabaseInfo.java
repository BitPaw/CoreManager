...
public class DatabaseInfo 
{
	 public static final int COUNTRY_EDITION = 1;
	    ...
	    static {
	        DatabaseInfo.formatter = new SimpleDateFormat("yyyyMMdd");
	    }
	    ...
	    public Date getDate() {
	        for (int i = 0; i < this.info.length() - 9; ++i) {
	            if (Character.isWhitespace(this.info.charAt(i))) {
	                final String dateString = this.info.substring(i + 1, i + 9);
	                try {
	                    synchronized (DatabaseInfo.formatter) {
	                        // monitorexit(DatabaseInfo.formatter)
	                        return DatabaseInfo.formatter.parse(dateString);
	                    }
	                }
