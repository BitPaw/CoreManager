...
public class CommandMaterial extends MaterialCommand implements ICommand
{
    ...
    public String reactBadArgsSize(final String usedCmd) {
        return ChatColor.DARK_RED + DigitalClockSystem.getMessagePrefix() + ChatColor.RED + " Correct usage: '/" + usedCmd + " material <name> <material id:data>'";
    }
    ...
    public void process(final DigitalClockSystem main, final Player player, final String[] args) {
        ...
        final String oldmat = clock.getMaterial().name().toLowerCase().replace("_", " ");
        if (super.isUsableNumber(args[2])) {
            final Material m = Material.getMaterial(args[2].toString());
            if (m.isSolid()) {
                player.sendMessage(ChatColor.DARK_GREEN + DigitalClockSystem.getMessagePrefix() + ChatColor.GREEN + " Your clock '" + args[1] + "' changed material from " + oldmat + " to " + clock.changeMaterial(m.toString(), 0).name().toLowerCase().replace("_", " "));
            }
            else {
            ...
            if (super.isUsableNumber(matdata[1])) {
                if (super.isUsableNumber(matdata[0])) {
                    ...
                    if (i.isSolid()) {
                        player.sendMessage(ChatColor.DARK_GREEN + DigitalClockSystem.getMessagePrefix() + ChatColor.GREEN + " Your clock '" + args[1] + "' changed material from " + oldmat + " to " + clock.changeMaterial(i.toString(), Integer.parseInt(matdata[1])).name().toLowerCase().replace("_", " "));
                    }
                    else {
            ...
            try {
                ...
                if (m != null) {
                    if (m.isSolid()) {
                       // player.sendMessage(ChatColor.DARK_GREEN + DigitalClockSystem.getMessagePrefix() + ChatColor.GREEN + " Your clock '" + args[1] + "' changed material from " + oldmat + " to " + clock.changeMaterial(m.toString(), 0), 0).name().toLowerCase().replace("_", " ")));
                    }
                    else {
                        player.sendMessage(ChatColor.DARK_RED + DigitalClockSystem.getMessagePrefix() + ChatColor.RED + " You can't use " + m.name().toLowerCase().replace("_", " ") + " as a material, because it is not a block (cuboid shape)!");
                    }
