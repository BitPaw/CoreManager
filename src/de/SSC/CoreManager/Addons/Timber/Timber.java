...
public class Timber
{
	...
	private HashMap<Player, Block[]> _trees;
	...
	{
		return _bukkitUtility.BlockUtility.IsWoodLog(block);
	}
	
	private boolean CheckIfLeeves(Block block)
	{
		return _bukkitUtility.BlockUtility.IsLeaveBlock(block);
	}
	...
	private boolean breaksTool(Player player, ItemStack item)
	{
		...
		if(player.getGameMode() != GameMode.CREATIVE)		
		{
			if (item != null)
			{
				isItemAxe = _bukkitUtility.ItemUtility.IsItemAxe(item);			
	...
	*/
	...
	{		
		...
		{								
			hasPermission = _permissionSystem.HasPlayerPermissionToBreakBlock(player);
			hasTool = _bukkitUtility.ItemUtility.IsItemAxeEquiped(player);						
			hasPermissionToUseTimber = _permissionSystem.HasPlayerPermission(player, TimberPermission.Use.toString());
			
			if (hasPermission && hasTool && hasPermissionToUseTimber) 
			{
